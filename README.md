...
## Before / After: Holdout Model Outputs

15 holdout quotes (unseen during training) run through both the original adapter and
the SPO-trained adapter. Full set: [`data/SEEING_IS_BELIEVING_EXAMPLES.md`](data/SEEING_IS_BELIEVING_EXAMPLES.md).

Three representative examples follow. Each shows the before/after side-by-side with a
brief annotation of which failure mode was fixed.
...
*Before: 11 entailed premises, repetitive, garbled casing (`obsERVED`, `oberved`), throughline off-topic. After: 3 clean premises, correct SPO structure, single-sentence throughline on the right subject.*
...
*Before: entailed section polluted with negatives ("would not change earth"), no throughline. After: on-topic entailed premises only, throughline present.*
...
| Artifact | Contents |
|---|---|
| [`data/SEEING_IS_BELIEVING_EXAMPLES.md`](data/SEEING_IS_BELIEVING_EXAMPLES.md) | All 15 holdout quotes — before/after outputs |
| `data/examples_training_format.jsonl` | Same examples in training-ready JSONL |
| `data/sample_quotes.txt` | Easy starter input set |
| `data/train_clean_for_model_967.jsonl` | Cleaned 967-quote corpus used in prior work |
