...
### 11. Extracted triplet fields should be verbatim spans, not LLM paraphrases

When an extraction prompt asks the model to pull facts from a source quote, the natural failure mode is paraphrase: the model produces semantically correct text that cannot be located as a substring of the original. This is invisible to all the existing reward components (format, headers, confidence, tags) — they only care about structure, not grounding.

**Fix:** Explicitly instruct the model in the prompt: subject and object in the Entailed Premises section MUST be verbatim text from the source quote. If the original text is ambiguous, the model may add a parenthetical transliteration immediately after: `verbatim text (clarification)`. Non-entailed premises and the throughline are exempt — they are synthesised context.

**SPO reward signal:** Add a section-targeted verbatim faithfulness bonus (`+0.05 × ratio`) that applies only to the Entailed Premises section. For each triplet, strip parenthetical transliterations from subject/object, then check each base term against the source quote via `find_span()`. The fraction of verbatim components drives the bonus.

### 12. SPO reward components can and should be section-targeted

The output of a structured reasoning model has multiple sections with different quality axes: Entailed Premises must be extractively faithful to the source; Non-Entailed Premises can be loosely contextual; the Throughline must be coherent, not verbatim. A uniform reward function applied to the whole output cannot distinguish these.

**Pattern:** Parse the output into named sections before scoring. Apply different reward checks per section:
- Entailed Premises → verbatim span faithfulness (extractive)
- Non-Entailed Premises → uniqueness, predicate specificity (diversity)
- Throughline → coherence, non-tautology (reasoning quality)
- All headers → exact format match (structure)

This is how SPO can teach the model to behave differently in different parts of its output — not through prompt alone, but through section-differentiated reward gradients.

### Takeaway

For any RL-from-feedback training loop: (1) generation controls must prevent degenerate outputs before rewards are ever computed, (2) reward functions must explicitly penalise known failure modes rather than only rewarding the ideal case, (3) reward signals computed from gold data are always suspect — check that the distribution of rewards across your training set actually varies before assuming SPO is doing anything useful, (4) offline preference optimisation cannot correct a habit the model has never been penalised for producing — if the failure mode is a specific generated token sequence, only online generation-and-penalise RL can reliably fix it, (5) for small models, multi-regimen training on semantically overlapping formats requires stratified sampling across (regimen × prompt-length) strata — without it, whichever regimen dominates the data mix will corrupt the shared header vocabulary for the other regimens, (6) post-training evaluation scripts must mirror the exact prompt-format pipeline used during training — a chat-template mismatch produces NO_HEADER silently, (7) `repetition_penalty` corrupts structured headers whenever those headers appear verbatim in the prompt instruction list — use `no_repeat_ngram_size=6` instead, (8) eval pipeline generation params must be identical to inference params — a README fix that never propagates to the eval script produces `avg_quality=0.0` silently, (9) evidence tag rewards must be binary — partial credit for missing tags leaks reward and degrades the tagging incentive, (10) one epoch of SPO is not enough for quality generalisation — validate on out-of-distribution inputs at each epoch boundary not just on holdout metrics, (11) extraction prompts must explicitly mandate verbatim text for grounded fields — parenthetical transliterations are the right escape hatch, and (12) SPO reward components should be section-targeted: each output section has different quality axes, and uniform whole-output scoring cannot teach section-differentiated behaviour.
